...
sp.restart.alg <- function(suffstat, intsuffstat, inttargets, alpha){
	...
	start.order <- lapply(1:50, function(x) sample(1:p, p, replace=F))
...
grspdag.list <- lapply(1:dagnum, function(i) {
	#suffstat <- list(C=cor(data.list[[i]][[1]]), n=nrow(data.list[[i]][[1]]))
	#intsuffstat <- lapply(1:intnum, function(t) list(C=cor(data.list[[i]][[t+1]]), n=nrow(data.list[[i]][[t+1]])))
	#inttargets <- t.list[[i]][2:(intnum+1)]
	#grspdag <- sp.restart.alg(suffstat, intsuffstat, inttargets, alpha)
	#dag2essgraph(as(grspdag, "graphNEL"), targets=t.list[[i]])
	suffstat <- list(C=cor(data.list[[i]][[1]]), n=nrow(data.list[[i]][[1]]))
	intsuffstat <- lapply(2:length(t.list[[i]]), function(t) list(C=cor(data.list[[i]][[t]]), n=nrow(data.list[[i]][[t]])))
	inttargets <- t.list[[i]][2:length(t.list[[i]])]
	grspdag <- sp.restart.alg(suffstat, intsuffstat, inttargets, alpha)
	dag2essgraph(as(grspdag, "graphNEL"), targets=t.list[[i]])
})
save(grspdag.list, dag.list, t.list, file=paste("result/", basename(args[4]), "_alpha_", toString(alpha), "_restart.rda", sep=""))
